...
> توجه تون رو جلب میکنم یه یه نکته! اگه بعد از براکت بخواید از کلمه کلیدی `then` استفاده کنید باید بعد از شرططون از `;` استفاده کنید و اگه `then` رو بزارید توی خط بعدی دیگه به `;` نیاز ندارید (الزامی نیست، ولی کلا بهتره که همیشه بزاریدش دیگه)
